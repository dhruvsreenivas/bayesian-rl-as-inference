...
from rl import environment as atari_environment
from rl import vapor_lite_atari
from rl.vapor_lite_atari import builder as atari_vapor_lite_builder
...
NUM_ACTOR_STEPS = flags.DEFINE_integer(
    "num_steps", 1_000_000, "Number of environment steps to run the agent for."
)
...
def build_experiment_config():
    """Builds Atari VAPOR-lite experiment config which can be executed in different ways."""
    ...
    def env_factory(seed):
        del seed
        return atari_environment.make_atari_environment(
            level=env_name,
            sticky_actions=True,
    ...
    lr_schedule = optax.linear_schedule(2e-4, 0.0, num_learner_steps)
    config = vapor_lite_atari.AtariVAPORLiteConfig(
    ...
    return experiments.ExperimentConfig(
        builder=atari_vapor_lite_builder.AtariVAPORLiteBuilder(config),
        environment_factory=env_factory,
        network_factory=vapor_lite_atari.make_networks,
        seed=SEED.value,
