...
def target_entropy_from_env_spec(
    ...
    if target_entropy_per_dimension is None:
        if not isinstance(spec.actions, specs.BoundedArray) or isinstance(
            ...
            raise ValueError(
                "Only accept BoundedArrays for automatic "
                f"target_entropy, got: {spec.actions}"
            )
        if not onp.all(spec.actions.minimum == -1.0):
            raise ValueError(f"Minimum expected to be -1, got: {spec.actions.minimum}")
        if not onp.all(spec.actions.maximum == 1.0):
            raise ValueError(f"Maximum expected to be 1, got: {spec.actions.maximum}")
